Feature: Update Topology Deployment
  ...
  Scenario: Change a node type should fail
    ...
    And I create a new application with name "updatableapp" and description "First version of my application with apache" based on the template with name "UpdatableTopologyFailInitial"
    ...
    And I create an application version for application "updatableapp" with version "0.2.0-SNAPSHOT", description "A new version where Apache is now for type is now mongo.", topology template id "UpdatableTopologyFailTypeUpdated:0.1.0-SNAPSHOT" and previous version id "null"
    And I should receive a RestResponse with no error
  ...
  Scenario: Add a scalable compute should fail
    ...
    And I create an application version for application "updatableapp" with version "0.2.0-SNAPSHOT", description "A new version where a scalable compute is added.", topology template id "UpdatableTopologyFailScalable:0.1.0-SNAPSHOT" and previous version id "null"
    And I should receive a RestResponse with no error
    And I update the application environment named "Environment" with values
      | currentVersionId | 0.2.0-SNAPSHOT |
    And I Set a unique location policy to "Mount doom orchestrator"/"Thark location" for all nodes
  ...
  Scenario: Change an hostedOn should fail
    ...
    And I create an application version for application "updatableapp" with version "0.2.0-SNAPSHOT", description "A new version where a hostedOn has been changed.", topology template id "UpdatableTopologyFailChangeHostedOn:0.1.0-SNAPSHOT" and previous version id "null"
    And I should receive a RestResponse with no error
    And I update the application environment named "Environment" with values
      | currentVersionId | 0.2.0-SNAPSHOT |
    And I Set a unique location policy to "Mount doom orchestrator"/"Thark location" for all nodes
