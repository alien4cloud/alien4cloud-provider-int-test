Feature: Lifecycle test
  ...
  Scenario: Scaling default 1 then 2,1,3,1 for A and B
    ...
    And I create a new application with name "test-lifecycle-1" and description "Test alien4cloud lifecycle: Scaling default 1 then 2,1,3,1 for A and B" based on the template with name "org.alien4cloud.tests.topologies.lifecycle"
...
# FIXME: why remove source is not called anymore ?
  ...
  Scenario: Should be able to read injected properties as input
    ...
    And I create a new application with name "test-lifecycle-3" and description "Test alien4cloud lifecycle: read injected properties as input" based on the template with name "org.alien4cloud.tests.topologies.lifecycle"
    And I Set a unique location policy to "cfy"/"aws" for all nodes
    And I substitute on the current application the node "ComputeRegistry" with the location resource "cfy"/"aws"/"Nano_Ubuntu"
    And I substitute on the current application the node "ComputeA" with the location resource "cfy"/"aws"/"Nano_Ubuntu"
    And I substitute on the current application the node "ComputeB" with the location resource "cfy"/"aws"/"Nano_Ubuntu"
